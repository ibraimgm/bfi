...
func (vm *BFVM) LoadFromStream(reader io.Reader) error {
	...
	buf.ReadFrom(parser.Parse(reader))
...
// WithSpecs returns a new VM instance, with the specified cell type and tape size
func WithSpecs(cellType CellType, tapeSize int) (*BFVM, error) {
	tape := make([]Cell, tapeSize)
	var err error

	for i := 0; i < tapeSize; i++ {
		tape[i], err = newCell(cellType)
...
// WithCellType returns a new VM instance, with the specified cell type
func WithCellType(cellType CellType) (*BFVM, error) {
	return WithSpecs(cellType, 3000)
}

// WithSize returns a new VM instance, with the specified tape size
func WithSize(tapeSize int) (*BFVM, error) {
	return WithSpecs(Cell8, tapeSize)
}

// New returns a new VM, with default specs (3000 8-bit cells)
func New() (*BFVM, error) {
	return WithSpecs(Cell8, 3000)
}
